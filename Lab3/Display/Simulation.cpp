...
ivec2 directionСalculation(MoveDirection direction, ivec2 nextPos) {
	switch (direction) {
...
void movePlayer() {
	...
	nextPlayerPos = directionСalculation(direction, nextPlayerPos);
		...
		for (int i = 0; i < 5; i++) {
			if (monsters[i] != nullptr) {
				...
				if (!(*monsters[i]).isMoving()) {
					...
					nextMonsterPosition = directionСalculation(direction, nextMonsterPosition);
	...
	if (bomb != nullptr) {
		...
		if (bombTime > 3.5) {
			...
			for (MoveDirection direction : directions) {
				...
				explosionRadius = directionСalculation(direction, explosionRadius);
