...
void Camera::setPosition(vec3 position)
{
	...
	r = length(position);
	// Куда столько комментов?
	// Лучше писать шорт-коммент после выражения (либо под ним, либо над)
	...
	// для определения угла используется
...
{
	angleX += degree;
	// Где фигурные скобки? У меня глаз сразу падает на 2 условия,
	// а не на выражение одного из условий
...
void Camera::rotateUpDown(float degree)
{
	...
	// Где фигурные скобки? У меня глаз сразу падает на 2 условия,
	// а не на выражение одного из условий
	if (angleY > 179) angleY -= 0.2;
	if (angleY < 1) angleY += 0.2;
	recalculatePosition();
}
